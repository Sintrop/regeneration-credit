...
contract Blockable {
  ...
  function getEpochForEra(uint256 era) public view returns (uint256) {
    require(era > 0, "Blockable: Era must be greater than 0");
  ...
  function nextEraIn(uint256 targetEra) public view returns (int256) {
    require(targetEra > 0, "Blockable: Target era must be greater than 0");
  ...
  function canWithdrawTimes(uint256 currentUserEra) public view returns (uint256) {
    require(currentUserEra > 0, "Blockable: User completed era must be greater than 0");
  ...
  modifier canWithdrawModifier(uint256 era) {
    require(canWithdraw(era), "Blockable: Action not allowed until user's current era has passed");
    _;
  }
}
