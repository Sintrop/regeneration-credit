...
describe("ValidatorPool", () => {
  ...
  beforeEach(async () => {
    ...
    await regenerationCredit.addContractPool(instance.target, args.totalValidatorPoolTokens);
  });
            ...
            context("when total of levels in era is 6", () => {
              ...
              context("when validator1 have 6 levels in era 1", () => {
                ...
                it("shoud withdraw 0 tokens to validator2", async () => {
                  ...
                  const balanceOf = await regenerationCredit.balanceOf(validator2Address);
          ...
          context("when is era 2", () => {
            context("when dont have withdraw from era 1", () => {
              ...
              context("when validator1 withdraw from era 1 and era 2", () => {
                ...
                it("validator pool balance must be 27500000000000000000000000", async () => {
                  const balance = await regenerationCredit.balanceOf(instance.target);

                  expect(balance).to.equal(27500000000000000000000000n);
                });

                it("validator1 balance must be 1250000000000000000000000", async () => {
                  const balanceOf = await regenerationCredit.balanceOf(validator1Address);

                  expect(balanceOf).to.equal(1250000000000000000000000n);
                });

                it("validator1 balance in era 1 must be 625000000000000000000000", async () => {
                  const balanceOf = await instance.eraTokens(1, validator1Address);

                  expect(balanceOf).to.equal(625000000000000000000000n);
                });

                it("validator1 balance in era 2 must be 625000000000000000000000", async () => {
              ...
              context("when validator2 withdraw from era 1 and era 2", () => {
                ...
                it("validator2 balance must be 1250000000000000000000000", async () => {
                  const balanceOf = await regenerationCredit.balanceOf(validator2Address);

                  expect(balanceOf).to.equal(1250000000000000000000000n);
                });

                it("validator2 balance in era 1 must be 625000000000000000000000", async () => {
                  const balanceOf = await instance.eraTokens(1, validator2Address);

                  expect(balanceOf).to.equal(625000000000000000000000n);
                });

                it("validator2 balance in era 2 must be 625000000000000000000000", async () => {
