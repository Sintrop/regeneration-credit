...
describe("ContributorPool", () => {
  ...
  let args = {
    totalContributorPoolTokens: "40000000000000000000000000",
    blocksPerEra: 12,
    halving: 12,
  };
  ...
  context("#addLevel", () => {
    context("with allowed caller", () => {
      context("when add level in era 1", () => {
        context("when contributor have 0 levels in era 1", () => {
          context("when add level", () => {
            beforeEach(async () => {
              await instance.addLevel(contr1Address, 1, eventId1);
              await instance.addLevel(contr2Address, 1, eventId2);
            });
        ...
        context("when contributors have levels in era 1", () => {
          beforeEach(async () => {
            await instance.addLevel(contr1Address, 1, eventId1);
            await instance.addLevel(contr1Address, 1, eventId2);

            await instance.addLevel(contr2Address, 1, eventId3);
            await instance.addLevel(contr2Address, 1, eventId4);
            await instance.addLevel(contr2Address, 1, eventId5);
          });

          context("when add level", () => {
            beforeEach(async () => {
              await instance.addLevel(contr1Address, 1, eventId6);
              await instance.addLevel(contr2Address, 1, eventId7);
            });
            ...
            it("eraLevels must have 3 level to contributor1", async () => {
              const eraLevels = await instance.eraLevels(1, contr1Address);
      ...
      context("when the same resource ID is processed twice", () => {
        ...
        beforeEach(async () => {
          ...
          await instance.connect(owner).addLevel(contr1Address, 1, duplicateResourceId);
        });
        ...
        it("should revert the second transaction with the same resource ID", async () => {
          ...
          await expect(instance.connect(owner).addLevel(contr1Address, 1, duplicateResourceId)).to.be.revertedWith(
        ...
        it("should have added the level only once", async () => {
          ...
          const contr1Levels = await instance.eraLevels(1, contr1Address);
    ...
    context("without allowed caller", () => {
      it("should return error message", async () => {
        await expect(instance.connect(contr1Address).addLevel(contr1Address, 1, eventId1)).to.be.revertedWith(
          "Not allowed caller"
        );
      });
    });
            ...
            context("when total of levels in era is 6", () => {
              context("when contr1 have 3 levels in era 1", () => {
                beforeEach(async () => {
                  await instance.addLevel(contr1Address, 1, eventId1);
                  await instance.addLevel(contr1Address, 1, eventId2);
                  await instance.addLevel(contr1Address, 1, eventId3);

                  await instance.addLevel(contr2Address, 1, eventId4);
                  await instance.addLevel(contr2Address, 1, eventId5);
                  await instance.addLevel(contr2Address, 1, eventId6);
              ...
              context("when contr1 have 6 levels in era 1", () => {
                beforeEach(async () => {
                  ...
                  await instance.addLevel(contr1Address, 1, eventId2);
                  await instance.addLevel(contr1Address, 1, eventId3);
                  await instance.addLevel(contr1Address, 1, eventId4);
                  await instance.addLevel(contr1Address, 1, eventId5);
                  await instance.addLevel(contr1Address, 1, eventId6);
          ...
          context("when is era 2", () => {
            context("when dont have withdraw from era 1", () => {
              beforeEach(async () => {
                ...
                await instance.addLevel(contr1Address, 1, eventId7);
                await instance.addLevel(contr1Address, 1, eventId8);
                await instance.addLevel(contr1Address, 1, eventId9);

                await instance.addLevel(contr2Address, 1, eventId10);
                await instance.addLevel(contr2Address, 1, eventId11);
                await instance.addLevel(contr2Address, 1, eventId12);
              });
              ...
              context("when contr1 withdraw from era 1 and era 2", () => {
                ...
                it("contr pool balance must be 36666666666666666666666668", async () => {
                  const balance = await regenerationCredit.balanceOf(instance.target);

                  expect(balance).to.equal(36666666666666666666666668n);
                });

                it("contr1 balance must be 1666666666666666666666666", async () => {
                  const balanceOf = await regenerationCredit.balanceOf(contr1Address);

                  expect(balanceOf).to.equal(1666666666666666666666666n);
                });

                it("contr1 balance in era 1 must be 833333333333333333333333", async () => {
                  const balanceOf = await instance.eraTokens(1, contr1Address);

                  expect(balanceOf).to.equal(833333333333333333333333n);
                });

                it("contr1 balance in era 2 must be 833333333333333333333333", async () => {
              ...
              context("when contr2 withdraw from era 1 and era 2", () => {
                ...
                it("contr2 balance must be 1666666666666666666666666", async () => {
                  const balanceOf = await regenerationCredit.balanceOf(contr2Address);

                  expect(balanceOf).to.equal(1666666666666666666666666n);
                });

                it("contr2 balance in era 1 must be 833333333333333333333333", async () => {
                  const balanceOf = await instance.eraTokens(1, contr2Address);

                  expect(balanceOf).to.equal(833333333333333333333333n);
                });

                it("contr2 balance in era 2 must be 833333333333333333333333", async () => {
