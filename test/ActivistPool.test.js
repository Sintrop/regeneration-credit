  ...
  describe("#withdraw", () => {
    context("with allowed caller", () => {
          ...
          context("when is era 2", () => {
            context("when dont have withdraw from era 1", () => {
              ...
              context("when activist1 withdraw from era 1 and era 2", () => {
                ...
                it("activist pool balance must be 36666666666666666666666668", async () => {
                  const balance = await regenerationCredit.balanceOf(instance.target);

                  expect(balance).to.equal(36666666666666666666666668n);
                });

                it("activist1 balance must be 1666666666666666666666666", async () => {
                  const balanceOf = await regenerationCredit.balanceOf(activist1Address);

                  expect(balanceOf).to.equal(1666666666666666666666666n);
                });

                it("activist1 balance in era 1 must be 833333333333333333333333", async () => {
                  const balanceOf = await instance.eraTokens(1, activist1Address);

                  expect(balanceOf).to.equal(833333333333333333333333n);
                });

                it("activist1 balance in era 2 must be 833333333333333333333333", async () => {
              ...
              context("when activist2 withdraw from era 1 and era 2", () => {
                ...
                it("activist2 balance must be 1666666666666666666666666", async () => {
                  const balanceOf = await regenerationCredit.balanceOf(activist2Address);

                  expect(balanceOf).to.equal(1666666666666666666666666n);
                });

                it("activist2 balance in era 1 must be 833333333333333333333333", async () => {
                  const balanceOf = await instance.eraTokens(1, activist2Address);

                  expect(balanceOf).to.equal(833333333333333333333333n);
                });

                it("activist2 balance in era 2 must be 833333333333333333333333", async () => {
      ...
      context("when cant withdraw", () => {
        it("should return error message", async () => {
          await expect(instance.withdraw(activist1Address, 1)).to.be.revertedWith("Blockable: Action not allowed until user's current era has passed");
        });
      });
    });
